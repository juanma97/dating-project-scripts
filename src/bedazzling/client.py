...
BEDAZZLING_MADRID_URL = "https://bedazzling.es/madrid/"

def fetch_and_parse_bedazzling_events(client: genai.Client) -> list[dict]:
    print(f"Fetching HTML from {BEDAZZLING_MADRID_URL}...")
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
    }
    response = requests.get(BEDAZZLING_MADRID_URL, headers=headers)
    ...
    prompt = f"""
    You are an expert data extraction algorithm. Read the following text extracted from the 'Bedazzling' speed dating website.
    ...
    1. EXTRACT STRICTLY 'Speed Dating' events.
    2. COMPLETELY EXCLUDE any events that are "Cenas" (dinners), "Cenas con afinidad", "Mesa Afinidad", or "Citas LGTBI" (unless they explicitly state Speed Dating). Do not include them in the output.
    ...
    - 'date': The date in YYYY-MM-DD format (convert the Spanish dates like 'Viernes 10 de Abril' to the correct upcoming date using the current year/next year context).
    - 'time': The time in HH:MM format (e.g., '20:15', '20:00').
    - 'city': "Madrid"
    - 'place': Specific name of the venue (e.g. "OCEANS EVENTS").
    - 'street_name': The street name of the venue (e.g. "C. de Gutierre de Cetina").
    - 'street_number': The street number (e.g. "10").
    - 'organizer': "Bedazzling"
    - 'source': "Bedazzling"
    - 'source_url': "{BEDAZZLING_MADRID_URL}"
    - 'image': null
    - 'min_age': Integer. Minimum age for this specific event/group.
    - 'max_age': Integer. Maximum age for this specific event/group.
    - 'sexual_orientation': 'straight', 'gay', 'lesbian', or 'all' based on text. For standard speed dating it is usually 'straight'.
